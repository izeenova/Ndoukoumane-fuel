...
export default function PersonnelPage() {
  ...
  const openEdit = (p: Personnel) => {
    setEditItem(p)
    setForm({ nom: p.nom, prenom: p.prenom, role: p.role, telephone: p.telephone || '', matricule: p.matricule || '' })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const data = await res.json()
    if (!res.ok) { setError(data.error || 'Erreur') } else { setShowModal(false); fetchPersonnel() }
    setSubmitting(false)
  }
  ...
  const inactifs = personnel.filter(p => !p.actif)
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400 text-sm">{actifs.length} membre{actifs.length !== 1 ? 's' : ''} actif{actifs.length !== 1 ? 's' : ''}</p>
        </div>
                ...
                <tr key={p.id} className={`hover:bg-slate-800/30 transition-colors ${!p.actif ? 'opacity-50' : ''}`}>
                  ...
                  <td className="px-5 py-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                      ...
                      {p.actif && (
                        <button onClick={() => handleDesactiver(p.id)}
          ...
          <div className="bg-[#1E293B] rounded-2xl border border-slate-700/50 w-full max-w-md shadow-2xl modal-animate">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm text-slate-300 mb-1.5">Matricule</label>
                  <input value={form.matricule} onChange={e => setForm(f => ({ ...f, matricule: e.target.value }))}
                    className="w-full bg-[#0F172A] border border-slate-700 rounded-xl px-4 py-2.5 text-white text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 font-mono"
                    placeholder="CHF-001" />
                </div>
                <div>
