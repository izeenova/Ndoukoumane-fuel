...
export default function VidangesPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Modal modifier vidange (admin) */}
      {showEdit && editItem && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-[#1E293B] rounded-2xl border border-slate-700/50 w-full max-w-md shadow-2xl">
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-700/50">
              <div>
                <h3 className="text-white font-semibold">Modifier la vidange</h3>
            ...
            <form onSubmit={handleEditSubmit} noValidate className="p-6 space-y-4">
              ...
              <div>
                ...
                <input type="number" value={editForm.cout} onChange={e => setEditForm(f => ({ ...f, cout: e.target.value }))}
                  className="w-full bg-[#0F172A] border border-slate-700 rounded-xl px-4 py-2.5 text-white text-sm focus:outline-none focus:ring-2 focus:ring-amber-500"
                  required min="1" />
              </div>
              <div>
